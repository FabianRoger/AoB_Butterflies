...
RMO_spat %>% 
  ...
  geom_sf(size = 0.5, aes(colour = `q = 0`))+
  facet_wrap(~round)+
...
grassland <- 
  Spec_cat %>% 
  filter(Grassland_spec) %>% 
  pull(SpeciesText)
  

RMO_spec_pooled_grass <- RMO_spec_pooled[,grassland]
...
RMO_spec_ind %>% 
  ...
  facet_wrap(~ round * SiteType, nrow = 1)+
  coord_flip()
  #theme_excel_new()
...
RMO_out_bio <- 
  RMO_mat_bio %>%
  t() %>% 
  iNEXT(datatype = "abundance", q = c(0,1),
                    nboot = 1) 
...
RMO_meta %>% 
  ...
  summarise(across(one_of( colnames(RMO_spec[,-1])), sum), .groups = "drop") %>% 
  pivot_longer(one_of(colnames(RMO_spec[,-1])), names_to = "Species", values_to = "Abundance") %>% 
  group_by(biotope_text, Species) %>% 
  summarise(Abundance = sum(Abundance)) %>% 
  group_by(Species) %>% 
  mutate(Abundance = Abundance / sum(Abundance)) %>% #standardise by total counts of Species
  group_by(Species) %>% 
  mutate(rel_abund = Abundance / sum(Abundance)) #get rel abundance 
